    ...
    <>
      ...
      >
        ...
        <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between shrink-0">
          ...
          <button
            onClick={onClose}
            className="lg:hidden text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300"
          >
            <X size={20} />
          </button>
        </div>
            ...
            filteredSessions.map((session) => {
              ...
              return (
                ...
                >
                  <MessageSquare
                    size={16}
                    className={`shrink-0 mt-0.5 ${currentSessionId === session.id ? 'text-blue-500' : 'text-slate-400'}`}
                  />
