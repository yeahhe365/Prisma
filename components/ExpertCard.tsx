...
const ExpertCard = ({ expert }: { expert: ExpertResult }) => {
  ...
  return (
    ...
    >
      {/* Header */}
      <div
        className={`p-4 border-b flex items-start gap-3 ${isDone ? 'bg-emerald-50/30 border-emerald-100' : 'bg-slate-50/50 border-slate-100'}`}
      >
        <div
          className={`mt-0.5 p-1.5 rounded-lg ${isWorking ? 'bg-blue-100 text-blue-600' : isError ? 'bg-red-100 text-red-600' : 'bg-slate-200 text-slate-600'}`}
        >
          <Bot size={18} />
        </div>
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2">
            ...
            {expert.temperature !== undefined && (
              <div
                className="flex items-center gap-0.5 px-1.5 py-0.5 rounded-full bg-slate-200/50 border border-slate-200 text-[9px] font-mono text-slate-500 shrink-0"
          ...
          <>
            {view === 'thoughts' && (
              <div className="prose prose-xs max-w-none">
                {expert.thoughts ? (
                  isWorking ? (
                    <pre className="text-slate-500 font-mono text-[11px] leading-relaxed whitespace-pre-wrap break-words">
                      {expert.thoughts}
                    </pre>
                  ) : (
            ...
            {view === 'output' && (
              <div className="prose prose-sm max-w-none">
                {expert.content ? (
                  isWorking ? (
                    <pre className="text-slate-700 text-xs leading-relaxed whitespace-pre-wrap break-words">
                      {expert.content}
                    </pre>
                  ) : (
                    <LazyMarkdownRenderer
